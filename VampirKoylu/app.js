...
function initApp() {
    ...
    window.onPlayerSelected = (targetId) => {
        if (!els.game.actionPanel.classList.contains('hidden') && targetId) {
            ...
            if (p && p.isAlive && targetId !== myId) {
                const isDedektif = gameState.players[myId].role === 'DEDEKTIF';
                if (!isDedektif) {
                    pendingActionTarget = targetId;
                    els.game.btnConfirm.classList.remove('hidden');
                    const nameDisplay = document.getElementById('action-selected-name');
                    if (nameDisplay) nameDisplay.textContent = 'Seçilen: ' + p.name;
                } else {
                    if (!Array.isArray(pendingActionTarget)) pendingActionTarget = [];
                    if (!pendingActionTarget.includes(targetId)) {
                        if (pendingActionTarget.length < 2) pendingActionTarget.push(targetId);
                        else { pendingActionTarget.shift(); pendingActionTarget.push(targetId); }
                    } else {
                        pendingActionTarget = pendingActionTarget.filter(id => id !== targetId);
                    }
                    if (pendingActionTarget.length === 2) els.game.btnConfirm.classList.remove('hidden');
                    else els.game.btnConfirm.classList.add('hidden');
                    
                    const nameDisplay = document.getElementById('action-selected-name');
                    if (nameDisplay) {
                        nameDisplay.textContent = 'Seçilen: ' + pendingActionTarget.map(id => gameState.players[id].name).join(', ');
                        if(pendingActionTarget.length === 0) nameDisplay.textContent = 'Seçilen: Hiçbiri';
                    }
                }
            }
        }
...
function initRoleBuilderUI() {
    if (!isHost) return;

    const spanSlotStatus = document.getElementById('slot-status');
    const modalRandomPool = document.getElementById('random-pool-modal');
    const btnEditPool = document.getElementById('btn-edit-random-pool');
    const btnClosePool = document.getElementById('btn-close-random-pool');

    function updateSlotStatus() {
        let totalSlots = 0;
        document.querySelectorAll('.role-count').forEach(el => {
            totalSlots += parseInt(el.textContent);
        });
        const playerCount = Object.keys(gameState.players).length || parseInt(els.lobby.playerCount.textContent) || 0;
        if(spanSlotStatus) {
            spanSlotStatus.textContent = `${totalSlots} / ${playerCount}`;
            if(totalSlots > playerCount && playerCount > 0) {
                spanSlotStatus.style.color = 'var(--danger)';
            } else {
                spanSlotStatus.style.color = 'var(--neon-purple)';
            }
        }
    }
    ...
    if (btnEditPool) {
        btnEditPool.addEventListener('click', () => {
            if(modalRandomPool) modalRandomPool.classList.remove('hidden');
        });
    }

    if (btnClosePool) {
        btnClosePool.addEventListener('click', () => {
            if(modalRandomPool) modalRandomPool.classList.add('hidden');
        });
    }
    
    // Call continuously to keep player count updated visually
...
function handleStartGame() {
    ...
    let randomCount = 0;
    document.querySelectorAll('.role-count').forEach(el => {
        const roleId = el.dataset.roleslot;
        const count = parseInt(el.textContent);
        if (count > 0) {
            if (roleId === 'RASTGELE') {
                randomCount = count;
            } else {
                for(let i=0; i<count; i++) roleDeck.push(roleId);
            }
        }
    });

    const totalSlots = roleDeck.length + randomCount;
    if (totalSlots > pCount) {
        showToast(`Hata: ${totalSlots} rol seçtiniz, ama sadece ${pCount} oyuncu var!`, "error");
        return;
    }

    const randomPool = [];
    document.querySelectorAll('.pool-toggle:checked').forEach(cb => {
        randomPool.push(cb.value);
    });

    if (randomCount > 0 && randomPool.length === 0) {
        showToast("Hata: Rastgele havuzunda işaretli rol yok!", "error");
        return;
    }

    const dtInput = document.getElementById('setting-discussion-time');

    gameState.settings = { 
        discussionTime: dtInput ? parseInt(dtInput.value) || 90 : 90,
        roleDeck: roleDeck,
...
function assignRoles() {
    ...
    let deck = [...gameState.settings.roleDeck];
    
    for(let i=0; i<gameState.settings.randomCount; i++) {
        const rIndex = Math.floor(Math.random() * gameState.settings.randomPool.length);
        deck.push(gameState.settings.randomPool[rIndex]);
    }
...
function renderGameScreen() {
    ...
    const nameDisplay = document.getElementById('action-selected-name');
    if (nameDisplay) nameDisplay.textContent = 'Seçilen: Hiçbiri';
...
function renderActionList(excludeSelf, maxSelect = 1) {
    // DOM'a kart basma işlemi iptal, artık 3D sahneden tıklanacak.
    els.game.actionPlayers.innerHTML = '';
}
