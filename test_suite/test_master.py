...
async def test_bagnam(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/Bagnam/index.html", wait_until="domcontentloaded", timeout=10000)
...
async def test_katiplik(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/Katiplik/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        # Select category from #category-select
        await page.wait_for_selector("#category-select option", timeout=5000)
        await page.evaluate("""() => {
        ...
        # Simulate typing 6 words in #text-input
...
async def test_kelime_avi(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/KelimeAvi/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        if await btn_adv.count() > 0:
            await btn_adv.click()
            await page.wait_for_timeout(200)
            await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "kelime_avi_adv_settings.png"))
            
        # Inject mock players to satisfy 3 player minimum and start game
        await page.evaluate("""() => {
            const engine = window.gameEngine || window.engine;
            if (engine) {
                engine.addPlayer('mock_p2', 'Oyuncu 2', false);
                engine.addPlayer('mock_p3', 'Oyuncu 3', false);
                engine.startGame({ totalRounds: 3, turnDuration: 45, ebeWinPts: 2, masumWinPts: 1, masumExtraPts: 1 });
            }
        }""")
        await page.wait_for_timeout(1000)
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "kelime_avi_gameplay.png"))
        record("KelimeAvi", "PASS", "Lobby, 3-Player setup, Advanced settings, Word Trap start")
    except Exception as e:
...
async def test_krono(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/Krono/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        await page.evaluate("""() => {
            const engine = window.gameEngine || window.engine;
            if (engine) {
                engine.addPlayer('mock_krono_p2', 'Rakip', false);
                if (engine.startGame) engine.startGame();
            }
            const btn = document.querySelector('#btn-start-game, #btn-start');
...
async def test_tabu(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/Tabu/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        # Add mock player and start game
        await page.evaluate("""() => {
            const engine = window.tabuNet ? window.tabuNet.engine : (window.gameEngine || window.engine);
            if (engine) {
                engine.addPlayer('mock_tabu_p2', 'Rakip A', false, 'B');
                engine.startGame({ rounds: 3, duration: 60, passLimit: 3, penalty: 1, category: 'Hepsi', minDifficulty: 1, maxDifficulty: 100 });
            }
        }""")
        await page.wait_for_timeout(1000)
...
async def test_bilgi_yarismasi(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/BilgiYarismasi/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        # Select all categories & start game with 2 players
        ...
        await page.evaluate("""() => {
            const engine = window.gameEngine || window.engine;
            if (engine) {
                engine.addPlayer('mock_bilgi_p2', 'RakipBilgin', false);
                engine.startGame({
                    turnDuration: 20,
                    roundCount: 5,
                    wrongPenalty: true,
        ...
        options = page.locator(".choice-btn")
...
async def test_cizbil(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/CizBil/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        await page.evaluate("""() => {
            const engine = window.gameEngine || window.engine;
            if (engine) {
                engine.addPlayer('mock_cizbil_p2', 'Izleyici', false);
                engine.startGame({
                    turnDuration: 60,
                    roundCount: 3,
                    wordCount: 3
        ...
        record("CizBil", "PASS", "Lobby, Canvas Drawing, Brush Tools, Game state")
    except Exception as e:
...
async def test_cizim_zinciri(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/CizimZinciri/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        # Start game
        await page.evaluate("""() => {
            if (window.networkState && window.startGame) {
                window.networkState.players['mock_z2'] = { name: 'Zincirci2', id: 'mock_z2' };
                window.startGame();
            }
        }""")
        await page.wait_for_timeout(1000)
...
async def test_gizli_kelimeler(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/GizliKelimeler/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        # Start game
        await page.evaluate("""() => {
            const engine = window.gameNet ? window.gameNet.engine : (window.gameEngine || window.engine);
            if (engine) {
                engine.addPlayer('mock_ajan_p2', 'Ajan2', false, 'blue');
                engine.startGame({ boardSize: 25, turnDuration: 90 });
            }
        }""")
        await page.wait_for_timeout(1200)
...
async def test_hizli_isim_sehir(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/HizliIsimSehir/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        await page.evaluate("""() => {
            const engine = window.gameEngine || window.engine;
            if (engine) {
                engine.addPlayer('mock_his_p2', 'Hizli2', false);
                engine.startGame({
                    roundCount: 3,
                    categories: ['İsim', 'Şehir', 'Hayvan', 'Bitki'],
                    endCondition: 'time',
                    endValue: 45
                });
            }
...
async def test_isim_sehir(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/IsimSehir/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        ...
        # Start game
        await page.evaluate("""() => {
            const btn = document.getElementById('btn-start-game') || document.querySelector('.btn-primary');
            if (btn) { btn.classList.remove('disabled'); btn.click(); }
        }""")
        await page.wait_for_timeout(1200)
...
async def test_vampir_koylu(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/VampirKoylu/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
...
async def test_gartic(browser):
    print("\n--- 14. Testing Gartic (Chat ÇizBil) ---", flush=True)
    page = await browser.new_page(viewport={"width": 1280, "height": 800})
    setup_page_monitoring(page, "Gartic")
    try:
        await page.goto(f"http://localhost:{PORT}/Gartic/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "gartic_landing.png"))
        
        # Fill channel-input and start
        await page.fill("#channel-input", "pairatest")
        ...
        if await canvas.count() > 0:
            ...
            if box:
                ...
                await page.mouse.up()
                await page.wait_for_timeout(300)
                await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "gartic_canvas_drawn.png"))
                
        record("Gartic", "PASS", "Landing, Streamer Channel input, Canvas Drawing, Stream UI")
    except Exception as e:
...
async def test_chat_tabu(browser):
    ...
    try:
        await page.goto(f"http://localhost:{PORT}/ChatTabu/index.html", wait_until="domcontentloaded", timeout=10000)
        await page.wait_for_timeout(300)
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "chat_tabu_landing.png"))
        
        # Fill channel-input and click single streamer mode
        await page.fill("#channel-input", "pairatest")
        await page.click("#btn-start-solo")
        await page.wait_for_timeout(1200)
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "chat_tabu_gameplay.png"))
        
        # Click skip or taboo button
        btn_skip = page.locator("#btn-skip")
        if await btn_skip.count() > 0 and await btn_skip.is_visible():
            await btn_skip.click()
            await page.wait_for_timeout(300)
            await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "chat_tabu_after_skip.png"))
            
        record("ChatTabu", "PASS", "Landing, Channel Config, Solo Streamer mode, Taboo card display")
    except Exception as e:
